...
class SendDataRepositoryImpl extends SendDataRepository {
  ...
  SendDataRepositoryImpl(
      {required this.dataRemoteDatasource, required this.networkInfo});

  @override
  Future<Either<Failure, bool>> sendData(int userId, int subId, String title,
      String text, List<SendModel> images) async {
    if (await networkInfo.isConnected) {
      try {
        final result = await dataRemoteDatasource.setData(
            userId, subId, title, text, images);
        return Right(result);
      } on ServerFailure {
